...
class PlotManager:
    ...
    def _write_data(self, estimated_position: Vector3r, estimated_velocity: Vector3r, target_speed: float):
        ...
        with open(self._csv_filename, "a") as csv_file:
            ...
            row = {
                self._fieldnames[0]: round(self._counter, 1),
                self._fieldnames[1]: round(estimated_position.x_val, 2),
                self._fieldnames[2]: round(estimated_position.y_val, 2),
                self._fieldnames[3]: round(estimated_velocity.x_val, 2),
                self._fieldnames[4]: round(estimated_velocity.y_val, 2),
                self._fieldnames[5]: round(self._car.get_kinematics().position.x_val, 2),
                self._fieldnames[6]: round(self._car.get_kinematics().position.y_val, 2),
                self._fieldnames[7]: round(self._car.get_kinematics().linear_velocity.x_val, 2),
                self._fieldnames[8]: round(self._car.get_kinematics().linear_velocity.y_val, 2),
                self._fieldnames[9]: round(target_speed, 2)
            }
